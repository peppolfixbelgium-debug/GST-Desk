    ...
    <Shell>
      <div className="mx-auto max-w-6xl px-4 py-16">
        ...
        <div className="mt-8 flex flex-col items-center gap-2">
          <div className="inline-flex rounded-full border border-line bg-surface p-1 text-xs shadow-sm" role="group" aria-label="Billing cycle">
            <button type="button" onClick={() => setCycle("monthly")} aria-pressed={cycle === "monthly"} className={`rounded-full px-5 py-2.5 font-medium transition ${cycle === "monthly" ? "bg-accent text-accent-fg" : "text-muted"}`}>Monthly</button>
            <button type="button" onClick={() => setCycle("annual")} aria-pressed={cycle === "annual"} className={`rounded-full px-5 py-2.5 font-medium transition ${cycle === "annual" ? "bg-accent text-accent-fg" : "text-muted"}`}>Annual</button>
          </div>
          <p className="text-xs font-medium text-accent">Annual billing = 10 months paid · 2 months free</p>
        </div>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {PRICING.map((p) => {
            ...
            return (
              <article key={p.id} className={`relative flex flex-col rounded-lg border bg-surface p-6 ${p.id === "firm" ? "border-accent" : "border-line"}`}>
                {p.id === "firm" ? <p className="text-[11px] uppercase tracking-wide text-accent">{t(lang, "mostChosen")}</p> : null}
                {cycle === "annual" && isPaid ? <span className="absolute right-4 top-4 rounded-full bg-accent px-2.5 py-1 text-[10px] font-semibold text-accent-fg">SAVE 16.7%</span> : null}
                ...
                {cycle === "annual" && isPaid ? <p className="mt-1 text-xs font-medium text-accent">₹{monthlyEquivalent.toLocaleString("en-IN", { maximumFractionDigits: 0 })} effective / month · 2 months free</p> : null}
        ...
        <p className="mt-8 text-center text-xs text-muted">Choose monthly for flexibility, or annual and pay for 10 months while using the service for 12. Payment activation remains behind the account/billing integration.</p>
      </div>
    </Shell>
  );
}
