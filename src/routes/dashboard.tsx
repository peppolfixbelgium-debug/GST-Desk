...
function DashboardPage() {
  ...
  return <Shell><div className="mx-auto max-w-6xl px-4 py-10">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.2em] text-accent">Account</p><h1 className="mt-2 text-4xl tracking-tight">Welcome back, {user.displayName || "there"}.</h1><p className="mt-2 text-sm text-muted">Your GST Desk workspace, plan and document activity in one place.</p></div><div className="flex gap-2"><Link to="/converter"><Button>Fix an invoice <ArrowRight className="ml-2 size-4"/></Button></Link><Button variant="outline" onClick={() => void load()} disabled={loading}>{loading ? "Refreshing…" : "Refresh"}</Button></div></div>
    ...
    <div className="mt-8 grid gap-5 lg:grid-cols-[.9fr_1.5fr]">
      <aside className="rounded-2xl border border-line bg-surface p-5"><div className="flex items-center gap-4"><div className="grid size-14 place-items-center rounded-full bg-accent text-lg font-semibold text-accent-fg">{initials}</div><div className="min-w-0"><p className="truncate font-medium">{user.displayName || "GST Desk user"}</p><p className="truncate text-xs text-muted">{user.primaryEmail}</p></div></div><div className="my-5 h-px bg-line"/><div className="space-y-4 text-sm"><div className="flex items-center gap-3"><UserRound className="size-4 text-muted"/><span>Account profile</span><CheckCircle2 className="ml-auto size-4 text-accent"/></div><div className="flex items-center gap-3"><ShieldCheck className="size-4 text-muted"/><span>Authenticated session</span><CheckCircle2 className="ml-auto size-4 text-accent"/></div><div className="flex items-center gap-3"><CreditCard className="size-4 text-muted"/><span>{quota?.planName || "Plan"}</span><span className="ml-auto text-xs text-muted">{quota?.billingCycle || "—"}</span></div></div>{quota?.role === "admin" ? <div className="mt-6 rounded-xl border border-accent/40 bg-accent/5 p-4"><p className="text-xs uppercase tracking-wide text-accent">Founder access</p><p className="mt-1 text-sm">Complete product access is enabled for this admin account.</p><Link to="/mission-control" className="mt-3 inline-flex text-xs font-medium text-accent">Open Command Center <ArrowRight className="ml-1 size-3"/></Link></div> : null}</aside>
      <div className="space-y-5"><section className="rounded-2xl border border-line bg-surface p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.16em] text-muted">Current plan</p><h2 className="mt-1 text-2xl">{quota?.planName || "Loading…"}</h2><p className="mt-1 text-sm text-muted">{quota?.billingCycle === "annual" ? "Annual billing" : "Monthly billing"} · {quota?.status || "Active"}</p></div><Link to="/pricing"><Button variant="outline">View plans</Button></Link></div><div className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-line p-4"><p className="text-xs text-muted">Used</p><p className="mt-1 text-2xl">{quota?.used ?? "—"}</p></div><div className="rounded-xl border border-line p-4"><p className="text-xs text-muted">Remaining</p><p className="mt-1 text-2xl">{quota?.remaining ?? "—"}</p></div><div className="rounded-xl border border-line p-4"><p className="text-xs text-muted">Monthly limit</p><p className="mt-1 text-2xl">{quota?.limit ?? "—"}</p></div></div>{quota ? <><div className="mt-5 h-2.5 overflow-hidden rounded-full bg-line"><div className="h-full rounded-full bg-accent transition-[width]" style={{width:`${pct}%`}}/></div><div className="mt-2 flex justify-between text-xs text-muted"><span>{pct}% used</span><span>Resets on the 1st</span></div></> : null}</section><section className="grid gap-3 sm:grid-cols-3"><Link to="/converter" className="rounded-2xl border border-line bg-surface p-5 hover:border-accent"><FileCheck2 className="size-5 text-accent"/><h3 className="mt-3 font-medium">Fix JSON</h3><p className="mt-1 text-xs leading-5 text-muted">Diagnose, safely fix, validate and download.</p></Link><Link to="/validate" className="rounded-2xl border border-line bg-surface p-5 hover:border-accent"><ShieldCheck className="size-5 text-accent"/><h3 className="mt-3 font-medium">Validate</h3><p className="mt-1 text-xs leading-5 text-muted">Check a payload without changing it.</p></Link><Link to="/bulk" className="rounded-2xl border border-line bg-surface p-5 hover:border-accent"><History className="size-5 text-accent"/><h3 className="mt-3 font-medium">Bulk tools</h3><p className="mt-1 text-xs leading-5 text-muted">Process multiple JSON files with the same safeguards.</p></Link></section></div>
    </div>
