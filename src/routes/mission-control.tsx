...
async function api<T>(path: string): Promise<T> { const response = await fetch(`${API}${path}`, { headers: { Accept: "application/vnd.github+json" } }); if (!response.ok) throw new Error(`GitHub API ${response.status}`); return response.json() as Promise<T>; }

function MissionControlPage() {
  const { user, isPending } = useCurrentUserState();
  const [issues, setIssues] = useState<Issue[]>([]); const [runs, setRuns] = useState<Run[]>([]); const [error, setError] = useState<string | null>(null); const [loading, setLoading] = useState(false);
  const refresh = async () => { setLoading(true); setError(null); try { const [nextIssues, workflow] = await Promise.all([api<Issue[]>("/issues?state=all&per_page=100"), api<{ workflow_runs: Run[] }>("/actions/runs?per_page=10")]); setIssues(nextIssues); setRuns(workflow.workflow_runs); } catch (e) { setError(e instanceof Error ? e.message : "Could not load GitHub state."); } finally { setLoading(false); } };
  ...
  const open = issues.filter((i) => i.state === "open"); const done = issues.filter((i) => i.state === "closed"); const moving = open.filter((i) => Date.now() - new Date(i.updated_at).getTime() <= 48 * 60 * 60 * 1000); const latest = runs[0];
