...
function FixerPage() {
  ...
  const loadQuota = async () => { try { setQuota(await getQuota()); } catch { setQuota(null); } };
  ...
  const onDownload = async () => {
    if (!invoice || !quota || quota.remaining <= 0) {
      ...
      setNotice("Monthly free quota used. It resets automatically on the 1st."); return;
    }
    try {
    ...
    <Shell>
      <div className="mx-auto max-w-6xl px-4 py-8">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="text-sm text-muted">{quota ? `${quota.used}/${quota.limit} used this month · ${remaining} left` : "Loading quota…"}</div>
        </div>
        {remaining <= 0 ? <div className="mt-4 rounded-md border border-line bg-surface px-4 py-3 text-sm">Free monthly quota used. It resets automatically on the 1st. <Link to="/pricing" className="text-accent underline">See plans</Link></div> : null}
        <div className="mt-6 grid gap-4 lg:grid-cols-3">
          ...
          <section className="rounded-lg border border-line bg-surface p-4"><h2 className="text-sm font-medium">Corrected JSON</h2><Textarea readOnly className="mt-3 h-[28rem]" value={invoice ? pretty(invoice) : ""} /><Button className="mt-3 w-full" disabled={!invoice || remaining <= 0} onClick={() => void onDownload()}>Download JSON</Button>{notice ? <p className="mt-2 text-xs text-muted">{notice}</p> : null}</section>
        </div>
      </div>
    </Shell>
  );
